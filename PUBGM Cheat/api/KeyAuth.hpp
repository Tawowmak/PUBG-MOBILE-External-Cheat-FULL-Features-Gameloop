...
namespace KeyAuth {
	...
	public:
		...
		bool license(std::string key) {
			...
			if (json[("success")])
			{
				// optional success message
				load_user_data(json[("info")]);
				Setting::LoginStatus = "Login Succes";
				Sleep(1000);
			}
			else
			{
				...
				{
					Setting::LoginStatus = "Key Is Banned";

				}
				else
				{
					Setting::LoginStatus = "Invaild Key";
				}
				return false;
			}
		}
	...
	private:
		...
		static std::string req(std::string data) {
			...
			curl_easy_setopt(curl, CURLOPT_URL, XorStr("https://pvtauth.site/api/1.0/").c_str());
